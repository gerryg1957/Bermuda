#!/usr/bin/env bash
# Stage already-built, signed 0.8.0 candidates. No publication or Git mutation.
set -euo pipefail
...
release_dir="$HOME/bermuda-packages/0.8.0"
...
python3 - "$repo_dir" "$release_dir" "$stage" <<'PY'
from pathlib import Path
import shutil, subprocess, sys, tomllib
repo, release, stage = map(Path, sys.argv[1:])
assert tomllib.loads((repo/'Cargo.toml').read_text())['package']['version'] == '0.8.0'
...
for source in [release/'RPMS/x86_64/bermuda-0.8.0-1.x86_64.rpm',
               release/'SRPMS/bermuda-0.8.0-1.src.rpm',
               release/'bermuda-0.8.0-x86_64-signed.flatpak', key]:
    if not source.is_file() or source.stat().st_size == 0:
        raise SystemExit(f'Missing or empty release input: {source}')
    shutil.copy2(source,stage/source.name)
shutil.copy2(repo/'docs/releases/0.8.0.md',stage/'RELEASE-NOTES.md')
...
(stage/'SOURCE-COMMIT.txt').write_text(
    ...
    'Verified Flatpak commit: 28b05883a7b75cb95ae26d9203b87a647fc6ba94f0128eb18c1ff0961fa299c6\n')
PY
cd "$stage"
sha256sum bermuda-0.8.0-1.x86_64.rpm bermuda-0.8.0-1.src.rpm \
    bermuda-0.8.0-x86_64-signed.flatpak bermuda-signing-key.asc \
